(() => {
  ...
  function beginChallenge() {
    resetChallengeState();
    appendOutput("DKR-1101: Docker is installed but no containers exist yet.\nMission: check the engine, pull nginx, run a detached container on port 80, inspect it, then stop and remove it cleanly.", "warning");
    showScreen("lab");
    startTimer();
  ...
  function resetChallengeState() {
    ...
    state.engineChecked = false; state.check = false;
    state.imagePulled = false; state.pull = false;
    state.containerRunning = false; state.run = false;
    state.logsChecked = false; state.stopped = false; state.removed = false; state.manage = false;
  ...
  function execute(raw) {
    ...
    const args = tokens;
    switch (command) {
      case "docker": handleDocker(args, raw); break;
      case "pwd": appendOutput("/home/dev"); break;
      case "whoami": appendOutput("dev"); break;
      case "hostname": appendOutput("docker-host-01"); break;
      case "uname": appendOutput(args.includes("-a") ? "Linux docker-host-01 6.12.0-55.el10.x86_64 x86_64 GNU/Linux" : "Linux"); break;
      case "date": appendOutput("Sun Aug 16 12:10:00 IST 2026"); break;
      ...
      case "man": appendOutput("DOCKER(1)\nNAME\n    docker - manage containers\nCOMMANDS\n    version info pull run ps images logs exec stop start rm rmi"); break;
      case "": break;
      default: appendOutput("bash: " + command + ": command not found", "error");
    }
  }

  function handleDocker(args, raw) {
    const sub = (args[0] || "").toLowerCase();
    if (sub === "version") {
      state.engineChecked = true;
      appendOutput("Client: Docker Engine - Community\n Version:           27.3.1\n API version:       1.47\nServer: Docker Engine - Community\n Engine:\n  Version:          27.3.1\n  API version:      1.47 (minimum version 1.24)\n Containerd Version: 1.7.22\n OS/Arch:           linux/amd64");
      markObjective("check", 20, "✓ Objective 1 passed — the Docker engine is running. +20 points");
      return;
    }
    if (sub === "info") {
      state.engineChecked = true;
      appendOutput("Client: Docker Engine - Community\n Server Version: 27.3.1\n Storage Driver: overlay2\n Containers: " + (state.containerRunning ? 1 : 0) + "\n  Running: " + (state.containerRunning ? 1 : 0) + "\n Images: " + (state.imagePulled ? 1 : 0) + "\n Server Version: 27.3.1");
      markObjective("check", 20, "✓ Objective 1 passed — the Docker engine is running. +20 points");
      return;
    }
    if (sub === "pull") {
      const image = args[1] || "";
      if (!image || image !== "nginx") { appendOutput('Error response from daemon: pull access denied for ' + (image || "?") + ', repository does not exist or may require authorization', "error"); return; }
      if (state.imagePulled) { appendOutput("Using default tag: latest\nlatest: Pulling from library/nginx\nDigest: sha256:a1b2c3d4...\nStatus: Image is up to date for nginx:latest"); return; }
      state.imagePulled = true;
      appendOutput("latest: Pulling from library/nginx\na1b2c3d4: Pull complete\nDigest: sha256:a1b2c3d4e5f6...\nStatus: Downloaded newer image for nginx:latest");
      markObjective("pull", 25, "✓ Objective 2 passed — the nginx image was pulled. +25 points");
      return;
    }
    if (sub === "run") {
      if (!state.imagePulled) { appendOutput("Unable to find image 'nginx:latest' locally\nPull it first with: docker pull nginx", "error"); return; }
      if (state.containerRunning) { appendOutput("docker: Error response from daemon: Conflict. The container name \"/web\" is already in use.", "error"); return; }
      const detached = /-d\b/.test(raw);
      const portMap = /-p\s+80:80/.test(raw) || /-p=80:80/.test(raw) || /--publish/.test(raw);
      if (!detached) { appendOutput("docker run requires -d (detached) for this task, and -p 80:80 to publish the port.", "error"); return; }
      if (!portMap) { appendOutput("docker run requires -p 80:80 to publish the web port.", "error"); return; }
      state.containerRunning = true;
      appendOutput("a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6");
      markObjective("run", 30, "✓ Objective 3 passed — the container is running detached on port 80. +30 points");
      return;
    }
    if (sub === "ps") {
      appendOutput("CONTAINER ID   IMAGE     COMMAND                  CREATED         STATUS         PORTS                NAMES");
      if (state.containerRunning) appendOutput("a1b2c3d4e5f6   nginx     \"/docker-entrypoint.…\"   2 minutes ago   Up 2 minutes   0.0.0.0:80->80/tcp   web");
      return;
    }
    if (sub === "images") {
      appendOutput("REPOSITORY   TAG       IMAGE ID       CREATED        SIZE");
      if (state.imagePulled) appendOutput("nginx        latest    abc123def456   3 weeks ago    187 MB");
      return;
    }
    if (sub === "logs") {
      const name = args[1] || "";
      if (name !== "web") { appendOutput("docker logs: no such container: " + name, "error"); return; }
      if (!state.containerRunning) { appendOutput("docker logs: container 'web' is not running", "error"); return; }
      state.logsChecked = true;
      appendOutput("10.0.0.1 - - [16/Aug/2026:12:10:00 +0000] \"GET / HTTP/1.1\" 200 612\n10.0.0.1 - - [16/Aug/2026:12:10:01 +0000] \"GET /favicon.ico HTTP/1.1\" 404 555");
      return;
    }
    if (sub === "stop") {
      const name = args[1] || "";
      if (name !== "web") { appendOutput("docker stop: no such container: " + name, "error"); return; }
      if (!state.containerRunning) { appendOutput("docker stop: container 'web' is not running", "error"); return; }
    ...
    if (sub === "rm") {
      const name = args[1] || "";
      if (name !== "web") { appendOutput("docker rm: no such container: " + name, "error"); return; }
      if (state.containerRunning) { appendOutput("Error response from daemon: You cannot remove a running container web. Stop it first (docker stop web).", "error"); return; }
      if (state.removed) { appendOutput("docker rm: no such container: web", "error"); return; }
      state.removed = true;
      appendOutput("web");
      markObjective("manage", 25, "✓ Objective 4 passed — the container was inspected, stopped and removed. +25 points");
      completeChallenge();
      return;
    }
    if (sub === "rmi") {
      const image = args[1] || "";
      if (image === "nginx") { appendOutput("Untagged: nginx:latest\nDeleted: sha256:abc123def456..."); return; }
      appendOutput("docker rmi: reference does not exist", "error");
      return;
    }
    if (sub === "start") {
      const name = args[1] || "";
      if (name === "web" && !state.containerRunning) { state.containerRunning = true; appendOutput("web"); return; }
      appendOutput("docker start: no such container: " + name, "error");
      return;
    }
    if (sub === "exec") {
      const name = args[1] || "";
      if (name !== "web") { appendOutput("docker exec: no such container: " + name, "error"); return; }
      if (!state.containerRunning) { appendOutput("docker exec: container 'web' is not running", "error"); return; }
      if (args.includes("cat") && args.includes("/usr/share/nginx/html/index.html")) {
        appendOutput("<!DOCTYPE html><html><head><title>Welcome to nginx!</title></head><body><h1>Welcome to nginx!</h1></body></html>");
        return;
      }
      if (args[args.length - 1] === "nginx" || (args.includes("-it") && args.includes("bash"))) {
        appendOutput("(exec session started — type 'exit' to leave)");
        return;
      }
      appendOutput("docker exec: unsupported command in this simulation");
      return;
    }
    appendOutput("docker: unknown command \"" + sub + "\". Use 'docker version|info|pull|run|ps|images|logs|exec|stop|start|rm|rmi'.");
  }
  ...
  function showHelp() {
    appendOutput(
      "Docker commands:\n" +
      "  docker version                     check client & server\n" +
      "  docker info                        show engine info\n" +
      "  docker pull nginx                  pull the nginx image\n" +
      "  docker run -d --name web -p 80:80 nginx   run detached on port 80\n" +
      "  docker ps                          list running containers\n" +
      "  docker images                      list images\n" +
      "  docker logs web                    show container logs\n" +
      "  docker exec -it web bash           open a shell in the container\n" +
      "  docker stop web / docker start web stop & restart\n" +
      "  docker rm web / docker rmi nginx   remove containers & images\n" +
  ...
  function appendCommand(cmd) {
    ...
    const u = document.createElement("span"); u.className = "prompt-user"; u.textContent = "dev@docker-host-01";
  ...
  function updateProgress() {
    const done = [state.check, state.pull, state.run, state.manage].filter(Boolean).length;
  ...
  function completeChallenge() {
    if (state.completed || !state.manage) return;
  ...
  $("#resetButton").addEventListener("click", () => { resetChallengeState(); appendOutput("Container task reset. Start by checking the Docker engine.", "info"); startTimer(); commandInput.focus(); });
  ...
  function makeCertificateId(name) {
    ...
    let h = 2166136261;
    const src = name + "|" + now.toISOString() + "|" + state.elapsedSeconds + "|docker-basics";
    for (let i = 0; i < src.length; i++) { h ^= src.charCodeAt(i); h = Math.imul(h, 16777619); }
    return "RCW-DKR-" + dp + "-" + (h >>> 0).toString(36).toUpperCase().padStart(7, "0").slice(-7);
  }
  ...
  function renderCertificate() {
    ...
    ctx.fillStyle = "#08233f"; ctx.font = "700 61px Georgia, serif"; ctx.fillText("Docker Container Champion", W / 2, 336);
    ...
    ctx.fillText("for successfully completing the RCW Docker basics challenge", W / 2, 543);
    ctx.fillText("by pulling, running, inspecting and stopping a container from the command line.", W / 2, 574);
    const bx = 206, by = 706;
    ...
    drawMeta(ctx, 660, 676, "CHALLENGE", "Docker container lifecycle");
    drawMeta(ctx, 660, 754, "STATUS", "All container objectives passed");
    ...
    ctx.textAlign = "right"; ctx.fillText("Containers · Challenge 01", W - 98, 921);
  }
  ...
  function certFilename(ext) { const s = state.learnerName.toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "learner"; return "rcw-docker-" + s + "." + ext; }
