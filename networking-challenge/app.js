(() => {
  ...
  function beginChallenge() {
    resetChallengeState();
    appendOutput("NET-1501: The workstation has no internet after a reconfiguration.\nMission: inspect the IP, verify the default route, test reachability, and analyse the subnet.", "warning");
    showScreen("lab");
    startTimer();
  ...
  function resetChallengeState() {
    ...
    state.inspect = false; state.route = false; state.ping = false; state.subnet = false;
    state.gatewayPinged = false; state.remotePinged = false;
  ...
  function execute(raw) {
    ...
    switch (command) {
      ...
      case "ipcalc": handleIpcalc(args); break;
      case "ss": handleSs(args); break;
      case "hostname": handleHostname(args); break;
      case "nslookup": appendOutput("Server:\t\t192.168.1.1\nAddress:\t192.168.1.1#53\n\nName:\texample.com\nAddress: 93.184.216.34"); break;
      case "dig": appendOutput("; <<>> DiG 9.18 <<>> example.com\n;; ANSWER SECTION:\nexample.com.\t3600\tIN\tA\t93.184.216.34"); break;
      case "traceroute": case "tracepath": appendOutput(" 1  192.168.1.1 (192.168.1.1)  1.2 ms\n 2  10.10.0.1 (10.10.0.1)  5.4 ms\n 3  93.184.216.34 (93.184.216.34)  24.1 ms"); break;
      case "pwd": appendOutput("/home/admin"); break;
      case "whoami": appendOutput("admin"); break;
      case "uname": appendOutput("Linux"); break;
      case "date": appendOutput("Sun Aug 16 13:00:00 IST 2026"); break;
      ...
      case "man": appendOutput("IP(8)\nNAME\n    ip - show / manipulate routing, devices, and addresses\nCOMMANDS\n    addr  route  link\n\nPING(8)\nNAME\n    ping - send ICMP ECHO_REQUEST to network hosts\n\nIPCALC(1)\nNAME\n    ipcalc - perform simple manipulation of IP addresses"); break;
      case "": break;
  ...
  function handleIp(args) {
    ...
    if (sub === "addr" || sub === "a" || sub === "address") {
      ...
      markObjective("inspect", 20, "✓ Objective 1 passed — the IP address was inspected. +20 points");
      return;
    }
    if (sub === "route" || sub === "r") {
      ...
      markObjective("route", 25, "✓ Objective 2 passed — the default route was verified. +25 points");
      return;
    }
    if (sub === "link" || sub === "l") {
  ...
  function handlePing(args) {
    ...
    const count = args.find((a) => a.startsWith("-c")) ? parseInt(args.find((a) => a.startsWith("-c")).replace("-c", ""), 10) : 4;
    if (!target) { appendOutput("ping: usage error: Destination address required", "error"); return; }
    if (target === "192.168.1.1") {
      state.gatewayPinged = true;
      appendOutput("PING 192.168.1.1 (192.168.1.1) 56(84) bytes of data.\n64 bytes from 192.168.1.1: icmp_seq=1 ttl=64 time=1.1 ms\n64 bytes from 192.168.1.1: icmp_seq=2 ttl=64 time=0.9 ms\n\n--- 192.168.1.1 ping statistics ---\n2 packets transmitted, 2 received, 0% packet loss");
    } else if (target === "8.8.8.8" || target === "google.com" || target === "example.com") {
      state.remotePinged = true;
      appendOutput("PING " + target + " (" + (target === "8.8.8.8" ? "8.8.8.8" : "93.184.216.34") + ") 56(84) bytes of data.\n64 bytes from " + (target === "8.8.8.8" ? "8.8.8.8" : "93.184.216.34") + ": icmp_seq=1 ttl=56 time=24.1 ms\n64 bytes from " + (target === "8.8.8.8" ? "8.8.8.8" : "93.184.216.34") + ": icmp_seq=2 ttl=56 time=23.8 ms\n\n--- " + target + " ping statistics ---\n2 packets transmitted, 2 received, 0% packet loss");
    } else {
      appendOutput("ping: " + target + ": Name or service not known", "error");
      return;
    }
    if (state.gatewayPinged && state.remotePinged) {
      markObjective("ping", 25, "✓ Objective 3 passed — reachability to gateway and remote host confirmed. +25 points");
    }
  }

  function handleIpcalc(args) {
    const cidr = args.filter((a) => !a.startsWith("-")).pop() || "";
    if (!cidr) { appendOutput("ipcalc: usage: ipcalc <ip>/<prefix>", "error"); return; }
    const m = cidr.match(/^(\d+\.\d+\.\d+\.\d+)\/(\d+)$/);
    if (!m) { appendOutput("ipcalc: invalid IP/CIDR format. Example: ipcalc 192.168.1.10/24", "error"); return; }
    const ip = m[1], prefix = parseInt(m[2], 10);
    const parts = ip.split(".").map(Number);
    let hostBits = 32 - prefix;
    const mask = (~0 << hostBits) >>> 0;
    const net = (parts.reduce((a, b) => (a << 8) + b, 0) & mask) >>> 0;
    const bcast = (net | (~mask >>> 0)) >>> 0;
    const toIp = (n) => [n >>> 24, (n >> 16) & 255, (n >> 8) & 255, n & 255].join(".");
    const maskIp = toIp(mask);
    appendOutput("Address:   " + ip + "          " + cidr);
    appendOutput("Netmask:   " + maskIp + " = " + prefix + "");
    appendOutput("Network:   " + toIp(net) + "/" + prefix);
    appendOutput("Broadcast: " + toIp(bcast));
    appendOutput("HostMin:   " + toIp((net + 1) >>> 0));
    appendOutput("HostMax:   " + toIp((bcast - 1) >>> 0));
    markObjective("subnet", 30, "✓ Objective 4 passed — the subnet was analysed. +30 points");
    completeChallenge();
  }

  function handleSs(args) {
    appendOutput("State      Recv-Q Send-Q Local Address:Port  Peer Address:Port  Process\nLISTEN     0      128    0.0.0.0:22           0.0.0.0:*          users:((\"sshd\",pid=1420))\nESTAB      0      0      192.168.1.10:22      192.168.1.25:52144 users:((\"sshd\",pid=1520))");
  }

  function handleHostname(args) {
    if (args.includes("-I")) appendOutput("192.168.1.10");
    else if (args.includes("-f")) appendOutput("net-node-01.rcw.local");
    else appendOutput("net-node-01");
  }
  ...
  function showHelp() {
    appendOutput(
      "Networking commands:\n" +
      "  ip addr | ip a                  inspect IP addresses\n" +
      "  ip route | ip r                show routing table (look for 'default via')\n" +
      "  ip link                        show interfaces\n" +
      "  ping -c 2 192.168.1.1          ping the gateway\n" +
      "  ping -c 2 8.8.8.8              ping a remote host\n" +
      "  ipcalc 192.168.1.10/24         analyse the subnet\n" +
      "  ss -tlnp                       list listening ports\n" +
      "  nslookup / dig / traceroute    DNS & path tools\n" +
  ...
  function appendCommand(cmd) {
    ...
    const u = document.createElement("span"); u.className = "prompt-user"; u.textContent = "admin@net-node-01";
  ...
  function updateProgress() {
    const done = [state.inspect, state.route, state.ping, state.subnet].filter(Boolean).length;
  ...
  function completeChallenge() {
    if (state.completed || !state.subnet) return;
  ...
  $("#resetButton").addEventListener("click", () => { resetChallengeState(); appendOutput("Network task reset. Start by inspecting the IP address.", "info"); startTimer(); commandInput.focus(); });
  ...
  function makeCertificateId(name) {
    ...
    let h = 2166136261;
    const src = name + "|" + now.toISOString() + "|" + state.elapsedSeconds + "|networking";
    for (let i = 0; i < src.length; i++) { h ^= src.charCodeAt(i); h = Math.imul(h, 16777619); }
    return "RCW-NET-" + dp + "-" + (h >>> 0).toString(36).toUpperCase().padStart(7, "0").slice(-7);
  }
  ...
  function renderCertificate() {
    ...
    ctx.fillStyle = "#08233f"; ctx.font = "700 61px Georgia, serif"; ctx.fillText("Networking Fundamentals Champion", W / 2, 336);
    ...
    ctx.fillText("for successfully completing the RCW networking fundamentals challenge", W / 2, 543);
    ctx.fillText("by inspecting addresses, verifying routes, testing reachability, and analysing subnets.", W / 2, 574);
    const bx = 206, by = 706;
    ...
    drawMeta(ctx, 660, 676, "CHALLENGE", "Network connectivity troubleshooting");
    drawMeta(ctx, 660, 754, "STATUS", "All network objectives passed");
    ...
    ctx.textAlign = "right"; ctx.fillText("Networking · Challenge 01", W - 98, 921);
  }
  ...
  function certFilename(ext) { const s = state.learnerName.toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "learner"; return "rcw-networking-" + s + "." + ext; }
