...
(function () {
  ...
  function validateDoc(d) {
    ...
    d.pages.forEach(p => {
      ...
      p.nodes = p.nodes.filter(n => n && typeof n === "object" && SH.get(n.type)).map(n => {
        ...
        if (typeof n.rot === "number" && isFinite(n.rot) && n.rot % 360 !== 0) o.rot = ((n.rot % 360) + 360) % 360; if (n.flipH) o.flipH = true; if (n.flipV) o.flipV = true; if (n.hidden) o.hidden = true;
      ...
      p.edges = p.edges.filter(e => e && e.from && e.to && ids.has(String(e.from.node)) && ids.has(String(e.to.node))).map(e => {
        const o = { id: String(e.id || C.uid("e")), from: { node: String(e.from.node), port: ["n", "e", "s", "w", "c"].includes(e.from.port) ? e.from.port : "auto" }, to: { node: String(e.to.node), port: ["n", "e", "s", "w", "c"].includes(e.to.port) ? e.to.port : "auto" }, points: Array.isArray(e.points) ? e.points.slice(0, 20).map(pt => ({ x: num(pt.x, 0), y: num(pt.y, 0) })) : [], label: String(e.label || "").slice(0, 500), srcLabel: String(e.srcLabel || "").slice(0, 120), dstLabel: String(e.dstLabel || "").slice(0, 120), style: {}, preset: C.PRESETS[e.preset] ? e.preset : "link", attrs: [], labelT: num(e.labelT, 0.5) };
  ...
  function exportCSV(pages) {
    ...
    pages.forEach(p => p.nodes.forEach(n => { const s = SH.get(n.type); if (!s || s.kind === "special" || s.kind === "text" || n.type === "anchor") return; const parent = n.parent ? (p.nodes.find(x => x.id === n.parent) || {}).label || "" : ""; const am = {}; (n.attrs || []).forEach(a => am[a.k] = a.v); inv += [p.name, p.level, n.id, s.name, s.cat, n.label.replace(/\n/g, " "), n.sub, parent.replace(/\n/g, " "), Math.round(n.x), Math.round(n.y), Math.round(n.w), Math.round(n.h), ...ak.map(k => am[k] || "")].map(csvEsc).join(",") + "\n"; }));
    let con = ["Page", "ID", "From", "From port label", "To", "To port label", "Label", "Media / type", "Routing", ...ek].map(csvEsc).join(",") + "\n";
    pages.forEach(p => p.edges.forEach(e => { const a = p.nodes.find(n => n.id === e.from.node), b = p.nodes.find(n => n.id === e.to.node); if (!a || !b) return; const am = {}; (e.attrs || []).forEach(x => am[x.k] = x.v); con += [p.name, e.id, (a.type === "anchor" ? "(free end)" : a.label || a.type).replace(/\n/g, " "), e.srcLabel, (b.type === "anchor" ? "(free end)" : b.label || b.type).replace(/\n/g, " "), e.dstLabel, e.label.replace(/\n/g, " "), (C.PRESETS[e.preset] || C.PRESETS.link).name, (e.style || {}).router || "orthogonal", ...ek.map(k => am[k] || "")].map(csvEsc).join(",") + "\n"; }));
    const bom = "\ufeff";
  ...
  const CHECKS = {
    run(focus) {
      ...
      const real = nodes.filter(n => { const s = SH.get(n.type); return s && s.kind !== "special" && s.kind !== "text" && n.type !== "anchor" && !s.noLabel && !["basic", "lines"].includes(s.cat); }); // decorative shapes (basic geometry, lines, markers) are exempt from naming checks
