(() => {
  ...
  function executeCommand(rawCommand) {
    ...
    switch (command) {
      ...
      case "hostname": appendOutput("data-node-01"); break;
  ...
  function handleCat(args) {
    ...
    if (!target.includes("orders.db")) { appendOutput("cat: " + target + ": No such file or directory", "error"); return; }
    const isLink = target.includes("/backup/");
    if (isLink && !state.hardLinkExists) { appendOutput("cat: " + target + ": No such file or directory", "error"); return; }
