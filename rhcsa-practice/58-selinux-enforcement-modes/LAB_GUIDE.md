...
**RHCSA Certification Practice · Task 58 of 62 · Security**

## Purpose

Demonstrate a temporary permissive diagnostic transition, restore enforcing mode, and ensure enforcing remains configured after reboot.
...
|---|---:|---|
| Assess the starting state | 20 | Current SELinux mode inspected |
| Manage runtime and persistent SELinux modes | 60 | Runtime mode changed to permissive, Runtime enforcing mode restored, Persistent enforcing mode configured |
| Validate the resulting state | 20 | Runtime and persistent SELinux modes validated |
...
```bash
getenforce && grep '^SELINUX=enforcing' /etc/selinux/config
```
