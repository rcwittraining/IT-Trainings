...
**RHCSA Certification Practice · Task 59 of 62 · Security**

## Purpose

Identify the security contexts on the web content and running web processes, then compare the file against policy expectations.
...
|---|---:|---|
| Assess the starting state | 20 | Directory security contexts listed |
| Inspect and interpret SELinux contexts | 60 | Web process contexts listed, Expected policy context identified |
| Validate the resulting state | 20 | File and process context evidence validated |
...
```bash
ls -Zd /srv/web && ps -eZ | grep -q httpd_t
```
