...
**RHCSA Certification Practice · Task 52 of 62 · Users and Groups**

## Purpose

Reset the trainee credential and apply the required minimum, maximum, and warning intervals.
...
|---|---:|---|
| Assess the starting state | 20 | Existing password-aging policy inspected |
| Apply local password-aging policy | 60 | Local password changed, Minimum, maximum, and warning intervals set |
| Validate the resulting state | 20 | Password-aging policy validated |
...
```bash
chage -m 1 -M 90 -W 7 trainee
```

```bash
chage -l trainee | grep -E 'Minimum|Maximum|warning'
```
