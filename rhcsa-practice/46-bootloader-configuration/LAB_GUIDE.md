...
**RHCSA Certification Practice · Task 46 of 62 · System Maintenance**

## Purpose

Add persistent auditing to every installed kernel command line and confirm the setting across boot entries.
...
|---|---:|---|
| Assess the starting state | 20 | Default kernel identified |
| Modify persistent bootloader kernel arguments | 60 | Auditing argument added to all kernels |
| Validate the resulting state | 20 | Bootloader arguments validated |
...
```bash
grubby --info=ALL | grep -E '^args=.*audit=1'
```
