...
**RHCSA Certification Practice · Task 10 of 62 · Essential Tools**

## Purpose

Restrict a payroll export so only its owner can modify it and members of the audit group can read it.
...
|---|---:|---|
| Assess the starting state | 20 | Current ownership and mode inspected |
| Apply the required owner, group and mode | 60 | Audit group assigned, Owner and group permissions corrected |
| Validate the resulting state | 20 | Final permissions validated |
...
```bash
chgrp auditors /srv/payroll/export.csv
```

```bash
chmod 0640 /srv/payroll/export.csv
```
