...
**RHCSA Certification Practice · Task 20 of 62 · Running Systems**

## Purpose

Demonstrate controlled reboot scheduling, cancellation, and normal shutdown handling on the isolated maintenance node.
...
|---|---:|---|
| Assess the starting state | 20 | Pending system jobs inspected |
| Use supported system lifecycle operations | 60 | Normal reboot scheduled with notice, Scheduled reboot safely cancelled, Normal system shutdown requested |
| Validate the resulting state | 20 | Lifecycle request state validated |
...
```bash
shutdown -r +5 'RCW maintenance validation'
```
