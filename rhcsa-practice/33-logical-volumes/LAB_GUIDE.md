# LVM Logical Volume Lifecycle

**RHCSA Certification Practice · Task 33 of 62 · Local Storage**

## Purpose

Create the replacement application logical volume and delete the confirmed retired logical volume.
...
|---|---:|---|
| Assess the starting state | 20 | Logical volume inventory inspected |
| Create and remove logical volumes | 60 | Replacement logical volume created, Retired logical volume removed |
| Validate the resulting state | 20 | Logical volume state validated |
...
```bash
lvcreate -L 2G -n lvapp vgapp
```

```bash
lvremove -y /dev/vgapp/lvold
```

```bash
lvs vgapp/lvapp -o lv_name,lv_size --noheadings
```
